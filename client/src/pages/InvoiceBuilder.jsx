...
import { Plus, Trash2, FileText, Loader2, Info, Tag, Percent, Zap, ChevronDown, Upload, Paperclip, X } from 'lucide-react';

// Item row type configurations
const ROW_TYPES = [
  { value: 'item', label: 'Item / Service', sign: +1, color: 'text-slate-700', badge: 'bg-slate-100 text-slate-600', desc: 'Normal product or service' },
  { value: 'discount', label: 'Discount (−)', sign: -1, color: 'text-rose-600', badge: 'bg-rose-50 text-rose-600', desc: 'Deduct amount (negative)' },
  { value: 'charge', label: 'Extra Charge (+)', sign: +1, color: 'text-amber-700', badge: 'bg-amber-50 text-amber-700', desc: 'Add surcharge or handling' },
  { value: 'tax', label: 'Custom Tax (+)', sign: +1, color: 'text-indigo-700', badge: 'bg-indigo-50 text-indigo-700', desc: 'Add specific tax line' },
];

function getRowType(value) {
  return ROW_TYPES.find(r => r.value === value) || ROW_TYPES[0];
}

export default function InvoiceBuilder({ t = {} }) {
  const { activeShop, customers, postInvoice, products, addProductRecord } = useContext(AppContext);
  ...
  // Selected customer profile tracking
  ...
  const handleInvoiceSubmission = async (e) => {
    e.preventDefault();
    ...
    try {
      for (let i = 0; i < processedItems.length; i++) {
        ...
        if (existingProduct) {
          ...
          const createRes = await addProductRecord(name, 0, 'Auto-created from invoice');
    ...
    if (response.success) {
      ...
      toast.error(`Billing Engine: ${response.error || 'Failed to post invoice'}`);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in duration-150 p-2 sm:p-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          <h2 className="text-sm font-black text-slate-900 flex items-center gap-2">
            ...
            {t.invoiceBuilderConsole || 'Invoice Builder Console'}
          </h2>
          <p className="text-slate-500 text-[11px] mt-0.5 font-medium font-sans">
            {t.invoiceBuilderDesc || 'Generate print-ready formats with instant ledger balancing overrides'}
          </p>
        </div>
      ...
      <form onSubmit={handleInvoiceSubmission} className="space-y-6">
        ...
        <div className="bg-white border border-slate-200 shadow-xl rounded-2xl p-6 sm:p-10 md:p-12 space-y-8 relative overflow-hidden">
          ...
          {/* 1. Header: Logo & Business details on Left, INVOICE label & metadata on Right */}
          <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-slate-200/80 pb-6">
            ...
            {/* Invoice metadata (Invoice No, Date, etc.) */}
              ...
              <div className="grid grid-cols-2 md:flex md:flex-col gap-2.5 w-full text-xs font-semibold">
                <div className="flex flex-col md:items-end">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5 font-mono">
                    {t.invoiceNoLabel || 'Invoice / Serial No.'}
                  </span>
                  <input
                    type="text"
                    value={invoiceNo}
                    ...
                    required
                    className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-slate-800 focus:outline-none focus:border-blue-500 focus:bg-white text-left md:text-right w-full md:w-44"
                  />
                </div>
                <div className="flex flex-col md:items-end">
                  <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5 font-mono">
          ...
          {/* 2. Client Details (BILL TO / Customer) */}
            ...
            <div className="space-y-2">
              ...
              {selectedCustomer ? (
                ...
                <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider py-4 px-3 border border-dashed border-slate-200 rounded-xl text-center font-mono mt-2 bg-slate-50/20">
                  Select customer profile to display client details
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              {isGstActive && gstMode === 'WITH_GST' ? (
                <div className="bg-indigo-50/30 border border-indigo-100 rounded-xl p-4 space-y-2.5">
                  <span className="text-[9px] font-black text-indigo-700 uppercase tracking-wider block font-mono">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-24">
                      ...
                      <input
                        ...
                        className="w-full bg-white border border-indigo-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-slate-800 focus:outline-none focus:border-indigo-400 focus:bg-white text-center"
                      />
                    </div>
                ...
                <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-[11px] font-medium text-slate-500 flex items-start gap-2 font-sans">
                  ...
                  <div>
                    ...
                    No tax rows or CGST/SGST breakdowns will be added to this invoice or applied to the grand total.
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="space-y-4">
              ...
              {activeShop?.bankDetails && (
                <div className="space-y-1.5">
                  ...
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 text-[10px] font-mono font-semibold text-slate-600 leading-relaxed whitespace-pre-line">
                    {activeShop.bankDetails}
                  </div>
                </div>
              )}
              ...
              {activeShop?.invoiceTerms && (
                <div className="space-y-1.5">
                  ...
                  <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 text-[10px] font-sans text-slate-500 font-medium leading-relaxed whitespace-pre-line">
                    {activeShop.invoiceTerms}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block font-mono">
                  {t.invoiceAttachment || 'Invoice Attachment (Image/PDF/Doc)'}
                </span>
                {attachedImgUrl ? (
              ...
              <div className="bg-slate-50 border border-slate-150 rounded-2xl p-4.5 space-y-3 font-mono text-[11px] text-slate-500 text-left md:text-right">
                ...
                {isGstActive && gstMode === 'WITH_GST' && (
                  <div className="space-y-2 border-t border-slate-200/50 pt-2 mt-2">
                    ...
                    <div className="flex justify-between items-center font-bold text-indigo-700 border-t border-indigo-100 pt-1.5 mt-1">
                ...
                <div className="flex justify-between items-center border-t border-slate-200/50 pt-2">
                  <span className="font-bold text-slate-655 flex items-center gap-1">Advance Payment (−):</span>
                  <div className="relative w-28 shrink-0">
                    <span className="absolute left-2 top-1 text-slate-400">₹</span>
                    <input
                      type="number"
                ...
                <div className="border-t border-slate-200 pt-2.5 mt-2 flex justify-between items-baseline">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider font-mono">
                    {t.grandTotalReceivable || 'Grand Total (Receivable)'}
                  </span>
        ...
        >
          ...
          {t.finalizeInvoiceBtn ? t.finalizeInvoiceBtn.replace(' 💾', '') : 'Finalize Invoice & Update Balances'} 💾
        </button>
      </form>
    </div>
  );
