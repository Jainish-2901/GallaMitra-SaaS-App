...
export default function InvoiceEditModal({
  ...
  // ── Local form state (mirrors InvoiceBuilder) ──────────────────────────
  ...
  useEffect(() => {
    ...
    setCustomTaxRate(parseFloat(invoice.taxRate || 0));
    setAdvancePayment(parseFloat(invoice.advancePayment || 0));
    setPaymentMode(invoice.paymentMode || 'CASH');
    setDescription(invoice.description || '');
    setAttachedImgUrl(invoice.attachedImgUrl || '');

    const taxRate = parseFloat(invoice.taxRate || 0);
    if (taxRate > 0) { setIsGstActive(true); setGstMode('WITH_GST'); }
    else { setIsGstActive(false); setGstMode('WITHOUT_GST'); }

    // Parse items – pull out charges into customCharges, rest into items
    let parsed = [];
    try {
      parsed = typeof invoice.itemsJson === 'string' ? JSON.parse(invoice.itemsJson) : (invoice.itemsJson || []);
    } catch { parsed = []; }
  ...
  // ── GST auto-detect on customer select ────────────────────────────────
  const selectedCustomer = customers.find(c => c.id === selectedCustomerId);
  useEffect(() => {
    if (selectedCustomer) {
      if (selectedCustomer.gstin && selectedCustomer.gstin.trim() !== '') {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!selectedCustomerId) { toast.warning('Please select a customer.'); return; }
      ...
      <div className="min-h-screen w-full max-w-4xl mx-auto px-2 py-6">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl mb-4 px-5 py-3.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-black text-slate-900 font-sans">Edit Invoice {invoiceNo}</h2>
              <p className="text-[10px] text-slate-400 font-mono mt-0.5">Modify invoice details — same as creation form</p>
            </div>
          </div>
          {/* GST Toggle */}
          <div className="flex items-center gap-3">
          ...
          <div className="bg-white border border-slate-200 shadow-xl rounded-2xl p-6 sm:p-10 md:p-12 space-y-8 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-6 border-b border-slate-200/80 pb-6">
              ...
              <div className="flex flex-col items-start md:items-end space-y-4 md:text-right w-full md:w-auto shrink-0">
                <span className="text-2xl font-black tracking-widest text-slate-400 uppercase font-mono">
                  {isGstActive && gstMode === 'WITH_GST' ? 'TAX INVOICE' : 'BILL OF SUPPLY'}
                </span>
                <div className="grid grid-cols-2 md:flex md:flex-col gap-2.5 w-full text-xs font-semibold">
                  <div className="flex flex-col md:items-end">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5 font-mono">Invoice / Serial No.</span>
                    <input type="text" value={invoiceNo} onChange={e => setInvoiceNo(e.target.value)} required
                      className="bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-slate-800 focus:outline-none focus:border-blue-500 focus:bg-white w-full md:w-44" />
                  </div>
                  <div className="flex flex-col md:items-end">
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider block mb-0.5 font-mono">Invoice Date</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
              <div className="space-y-2">
                <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest block font-mono">BILL TO (CLIENT COMPANY)</span>
                ...
                {selectedCustomer ? (
                  <div className="text-[11px] text-slate-500 font-semibold space-y-1 p-3.5 bg-slate-50/50 border border-slate-100 rounded-xl mt-2 leading-relaxed font-sans">
                    ...
                    <p>Address: <span className="text-slate-800 font-normal whitespace-pre-line">{selectedCustomer.billingAddress || 'N/A'}</span></p>
                    {selectedCustomer.gstin && (
                      <p className="mt-1 font-mono text-[9px] font-black text-emerald-700 bg-emerald-50 border border-emerald-100 inline-block px-1.5 py-0.5 rounded uppercase">GSTIN: {selectedCustomer.gstin}</p>
                    )}
                  </div>
                ) : (
                  <div className="text-[10px] text-slate-400 font-bold uppercase tracking-wider py-4 px-3 border border-dashed border-slate-200 rounded-xl text-center font-mono mt-2">Select customer to display details</div>
                )}
              </div>
              <div className="space-y-3">
                {isGstActive && gstMode === 'WITH_GST' ? (
                  <div className="bg-indigo-50/30 border border-indigo-100 rounded-xl p-4 space-y-2.5">
                    <span className="text-[9px] font-black text-indigo-700 uppercase tracking-wider block font-mono">GST TAX CONFIGURATION</span>
                    <div className="flex items-center gap-3">
                      <div className="w-24">
                        ...
                        <input type="number" value={customTaxRate} onChange={e => setCustomTaxRate(parseFloat(e.target.value) || 0)}
                          className="w-full bg-white border border-indigo-200 rounded-lg px-2.5 py-1.5 text-xs font-mono font-bold text-slate-800 focus:outline-none focus:border-indigo-400 text-center" />
                      </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 text-[11px] font-medium text-slate-500 flex items-start gap-2 font-sans">
                    ...
                    <div><span className="font-bold text-slate-800 block">Non-GST Template Activated</span>No tax rows or CGST/SGST will be applied to this invoice.</div>
                  </div>
                )}
              </div>
            </div>
                    ...
                    <div key={index} className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        ...
                        <div>
                          ...
                          <div className="bg-blue-50 border border-blue-100 rounded-lg px-2 py-1.5 text-xs text-right font-mono font-black text-slate-800">₹{lineTot.toFixed(2)}</div>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div className="space-y-4">
                {activeShop?.bankDetails && (
                  <div className="space-y-1.5">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block font-mono">BANK ACCOUNT DETAILS</span>
                    <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 text-[10px] font-mono font-semibold text-slate-600 leading-relaxed whitespace-pre-line">{activeShop.bankDetails}</div>
                  </div>
                )}
                {activeShop?.invoiceTerms && (
                  <div className="space-y-1.5">
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block font-mono">TERMS & CONDITIONS</span>
                    <div className="bg-slate-50/50 border border-slate-100 rounded-xl p-3 text-[10px] font-sans text-slate-500 font-medium leading-relaxed whitespace-pre-line">{activeShop.invoiceTerms}</div>
                  </div>
                )}
                <div className="space-y-1.5">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block font-mono">INVOICE REMARKS / MEMO</span>
                  <textarea value={description} onChange={e => setDescription(e.target.value)}
                    placeholder="Additional remarks, payment instructions, notes…" rows={3}
                ...
                <div className="space-y-1.5">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block font-mono">Invoice Attachment (Image/PDF/Doc)</span>
                  {attachedImgUrl ? (
              ...
              <div className="flex flex-col justify-between space-y-6 md:text-right">
                ...
                {activeShop?.signatureUrl && (
                  <div className="flex flex-col items-center md:items-end space-y-1">
                    <img src={activeShop.signatureUrl} alt="Signature" className="max-h-12 object-contain bg-white p-0.5 rounded shadow-2xs" />
                    <div className="border-t border-slate-350 w-36" />
                    <p className="text-[8px] text-slate-400 font-bold uppercase tracking-widest">Authorized Signatory</p>
                  </div>
                )}
              </div>
            </div>
