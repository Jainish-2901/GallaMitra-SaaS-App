...
import PurchaseBillEditModal from './PurchaseBillEditModal.jsx';
...
export default function DocumentListsView({ mode, t = {} }) {
  ...
  // Typeahead search focused row state for edit modals
  const [focusedEditRowIndex, setFocusedEditRowIndex] = useState(null);
  const [focusedEditPbRowIndex, setFocusedEditPbRowIndex] = useState(null);
  ...
  const handleSaveInvoiceEdit = async (data) => {
    const res = await editInvoice(editingInvoice.id, data);
    if (res.success) {
  ...
  const handleSavePurchaseBillEdit = async (data) => {
    const res = await editPurchaseBill(editingPurchaseBill.id, data);
    if (res.success) {
    ...
    <div className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm flex-1 animate-in fade-in duration-150">
      ...
      {editingPurchaseBill && (
        <PurchaseBillEditModal
          purchaseBill={editingPurchaseBill}
          suppliers={suppliers}
          products={products}
          activeShop={activeShop}
          onClose={() => setEditingPurchaseBill(null)}
          onSave={handleSavePurchaseBillEdit}
          toast={toast}
          t={t}
        />
      )}
