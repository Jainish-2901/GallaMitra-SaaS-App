...
GallaMitra (ગલ્લામિત્ર) is a premium, feature-rich ERP platform designed for micro, small, and medium businesses (MSMEs) to track day-to-day business transactions, manage customer/supplier ledgers, construct professional invoices, and generate deep business analytics.
...
- **Dynamic SMTP Configuration & Diagnostics (New)**: Fully configurable primary SMTP credentials stored securely in database, eliminating static `.env` lockouts. Includes an interactive connection verification handshake tool to validate Google App Passwords or custom credentials in real-time.
- Interactive SVG Charts: Built a custom, zero-dependency charting library ([CustomCharts.jsx](file:///d:/SasS Projects/gallamitra/client/src/components/CustomCharts.jsx)) utilizing pure SVG vectors and React state. It renders Sales & Expenses double bars, Cash Flow Area trends, Payment Mode Donuts (using offset mathematical stroke arrays), and a semi-circle Profit Margin Gauge.
- Refactored Settings Dashboards: Re-engineered user profiles ([UserSettings.jsx](file:///d:/SasS Projects/gallamitra/client/src/pages/UserSettings.jsx)) and business settings ([BusinessSettings.jsx](file:///d:/SasS Projects/gallamitra/client/src/pages/BusinessSettings.jsx)) into a modern 2-column layout separating basic details from platform metadata, subscription cards, and custom signature pads.
- Framer Motion Ledger Modals: Upgraded [TransactionModal.jsx](file:///d:/SasS Projects/gallamitra/client/src/components/TransactionModal.jsx) with scale-fade spring animations, absolute visual currency inputs (`₹`), and adaptive color schemes matching Credit (emerald) or Debit (rose) flows.
